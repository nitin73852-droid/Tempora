import { RoomRow, RoomStateV2, RoomType, RoomDuration, Room } from '../types';
...
class RoomService {
  public async createRoom(
    ...
    if (duration === '30-min') {
      expiresAtStr = new Date(Date.now() + 30 * 60 * 1000).toISOString();
    } else if (duration === '1-hour' || duration === '1h') {
      expiresAtStr = new Date(Date.now() + 60 * 60 * 1000).toISOString();
    } else if (duration === '24-hour' || duration === '8h' || duration === '24h') {
      expiresAtStr = new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString();
      ...
      expiresAtStr = new Date(Date.now() + customDurationMinutes * 60 * 1000).toISOString();
    }
    ...
    await dbClient.execute({
      sql: `INSERT INTO rooms (id, room_name, room_type, duration, status, host_id, is_locked, last_seq, created_at, expires_at)
            VALUES (?, ?, ?, ?, ?, ?, 0, 0, ?, ?)`,
      args: [id, name, type, duration, 'active', hostId, createdAtStr, expiresAtStr || 'never'],
    });
  ...
  public async getRoom(id: string, activeMemberIds?: Set<string>): Promise<Room | null> {
    const roomRow = await this.getRoomRow(id);
    if (!roomRow) return null;

    let status = (roomRow.status as string) || 'active';
    const expiresAtStr = roomRow.expires_at as string | null;
    const expiresAtDate = expiresAtStr && expiresAtStr !== 'never' ? new Date(expiresAtStr) : undefined;

    if (status !== 'ended' && status !== 'destroyed' && expiresAtDate && new Date() > expiresAtDate) {
      status = 'destroyed';
      await this.deleteRoom(id);
      return null;
    }

    const dbMembers = await memberService.getRoomMembers(id);

    const members = dbMembers.map((m) => ({
      ...m,
      online: activeMemberIds ? activeMemberIds.has(m.memberId) : false,
    }));

    return {
      id: roomRow.id as string,
      name: roomRow.room_name as string,
      type: roomRow.room_type as RoomType,
      duration: roomRow.duration as RoomDuration,
      status: status as 'active' | 'ending' | 'ended' | 'destroyed',
      ...
      expiresAt: expiresAtStr === 'never' ? undefined : (expiresAtStr || undefined),
      members,
